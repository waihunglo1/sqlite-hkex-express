...
QUERY_START_DATE = ""  # e.g., '20260730'
QUERY_END_DATE = ""  # e.g., '20260730'
...
def percent_rank_inc(data_list: List[float], value: float) -> float:
...
def safe_pct_change(
    current_val: float, past_val: float, default: float = 0.0
) -> float:
...
def calculate_ppo_score(
    ...
    degrees = slope_to_degrees(slope)
...
def calculate_sctr_base(stats_dict: Dict[str, Any]) -> None:
    ...
    stats_dict["ppo01sctr"] = calculate_ppo_score(
...
def calculate_technical_indicators(
    price_history: List[Dict], stats_dict: Dict[str, Any]
) -> None:
    ...
    if n >= 20:
        ...
        stats_dict["roc20"] = safe_pct_change(closes.iloc[-1], closes.iloc[-21])
    if n >= 50:
    ...
    if n >= 125:
        stats_dict["roc125"] = safe_pct_change(closes.iloc[-1], closes.iloc[-126])
    if n >= 150:
...
def calculate_volume_profile(price_history: List[Dict], stats_dict: Dict):
    subset = price_history[:150]
    if not subset:
        return

    highs = np.array([float(x["high"]) for x in subset])
    lows = np.array([float(x["low"]) for x in subset])
    volumes = np.array([float(x["volume"]) for x in subset])
    ...
    stats_dict.update({
        ...
        "vp_bullish": float(counts[max_bin_idx] / 2),  # approximation
...
def calculate_statistics(
    symbol: str, query_date: str, warning_list: List[Dict]
    ...
    history_rows = sqliteDbHelper.fetchAllRows(historySql, params)

    statsSql = "SELECT * FROM DAILY_STOCK_STATS WHERE symbol = ? AND dt < ? ORDER BY dt DESC LIMIT 200"
    params = [symbol, query_date]
    stats_rows = sqliteDbHelper.fetchAllRows(statsSql, params)
    ...
    try:
        calculate_technical_indicators(price_history, stats_dict)
        calculate_sctr_base(stats_dict)
        calculate_volume_profile(price_history, stats_dict)
...
def normalize_relative_strength(
    query_date: str, price_stats_list: List[Dict[str, Any]]
) -> None:
    ...
    for ps in price_stats_list:
        ps["rs_priceOverSMA20"] = (
            percent_rank_inc(price_over_sma20, ps.get("priceOverSMA20", 0)) * 100
        )
        ps["rs_slopeSMA20"] = (
            percent_rank_inc(slope_sma20, ps.get("slopeSMA20", 0)) * 100
        )
        ps["rs_slopeSMA50"] = (
            percent_rank_inc(slope_sma50, ps.get("slopeSMA50", 0)) * 100
        )
        ps["rs_slopeSMA150"] = (
            percent_rank_inc(slope_sma150, ps.get("slopeSMA150", 0)) * 100
        )
    ...
    for ps in price_stats_list:
        ps["normalise_rs_v2"] = (
            percent_rank_inc(rs_norm_list, ps["normalise_rs"]) * 100
        )
        ps["sctr"] = percent_rank_inc(sctr_list, ps["sctr"]) * 100
...
def process_single_date(
    query_date: str, query_symbol: Optional[str] = None
) -> int:
    ...
    rows = sqliteDbHelper.fetchAllRows(sql, params)
    ...
    for idx, sym in enumerate(symbols, start=1):
        stats_dict = calculate_statistics(sym, query_date, warning_list)
        if stats_dict:
    ...
    normalize_relative_strength(query_date, price_stats_list)
    updated = sqliteDbHelper.insertOrReplacePriceStats(price_stats_list)
...
def process_data_local() -> None:
    """Main task execution flow."""

    if QUERY_DATE:
        process_single_date(QUERY_DATE, QUERY_SYMBOL or None)
    else:
        sql = """
            SELECT dt FROM ( 
        ...
        rows = sqliteDbHelper.fetchAllRows(sql)
        dates = [r["dt"] for r in rows]

        for dt in dates:
            logging.info(f"Processing date: {dt}")
            count = process_single_date(dt)
